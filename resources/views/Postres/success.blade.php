...
@section('content')
    ...
    <div class="card-body" style="background-color: black;">
        ...
        <a href="{{ route('Postres.index') }}" class="btn btn-primary"><i class="fa fa-angle-left"></i> Continuar comprando</a>
        <table class="table table-striped">
            <thead>
                <tr>
                    <th style="width: 30%;">Postre</th>
            ...
            <tbody>
                ...
                @if(session('carrito'))
                    @foreach(session('carrito') as $id => $details)
                        @php $total += $details['postre_precio'] * $details['quantity'] @endphp
                        <tr data-id="{{ $id }}">
                            <td data-th="Product">
                                <div class="row">
                                    <div class="col-sm-6">
                                        <img src="{{ asset('img') }}/{{ $details['postre_imagen'] }}" style="max-height: 100px;" width="50" height="50" class="img-responsive" />
                                    </div>
                                    <div class="col-sm-6">
                                        <p class="nomargin">{{ $details['nombre_postre'] }}</p>
                                    </div>
                                </div>
                            </td>
                            <td data-th="Precio">${{ $details['postre_precio'] }}</td>
                            <td data-th="Cantidad">{{ $details['quantity'] }}</td>
                            <td data-th="Subtotal" class="text-center">${{ $details['postre_precio'] * $details['quantity'] }}</td>
                        </tr>
                    @endforeach
                @endif
            </tbody>
